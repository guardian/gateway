...
export const getRedirectUrl = (
	...
	if (!fromURI && locationPathname.startsWith('/oauth2/')) {
		// set prompt=none to avoid showing the okta hosted login page again
		// in some flows such as Guardian Jobs, we rely on the abscense of a `prompt`
		// parameter to determine if the user has seen the Okta hosted login page yet
