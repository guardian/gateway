...
corepack pnpm run mock-server &
corepack pnpm run wait-on:mock-server
...
corepack pnpm run cypress open --env $CI_ENV --config '{"e2e":{"specPattern":["**/mocked-okta/**/*.cy.ts"]}}' --e2e --browser chrome
