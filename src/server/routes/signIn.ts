...
router.get(
	'/signin/:social',
	handleAsyncErrors(
		async (req: Request<{ social: string }>, res: ResponseWithRequestState) => {
			const socialIdp = req.params.social as SocialProvider;
			const signInEmail = req.query.signInEmail as string | undefined;
			...
			// GOOGLE ONE TAP FLOW
			// If the provider is `google` and we have a `signInEmail` parameter, assume
			// that we're going down the Google One Tap sign in route. We cannot use the IDX API in
			// this route due to a limitation where we can't(easily) set parameters for the Google IdP
			// authentication URL as it is generated by Okta.
			if (signInEmail && socialIdp === 'google') {
				return await performAuthorizationCodeFlow(req, res, {
					idp,
					closeExistingSession: true,
					scopes: scopesForAuthentication,
