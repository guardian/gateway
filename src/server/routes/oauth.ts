...
import { setUserFeatureCookie } from '@/server/lib/user-features';
...
router.get(
  ...
  handleAsyncErrors(async (req: Request, res: ResponseWithRequestState) => {
    try {
      ...
      if (tokenSet.access_token) {
        // get the access token so we can use it to get the user's attributes
        // and set any required feature cookies
        try {
          await setUserFeatureCookie(
            tokenSet.access_token,
            res,
            res.locals.requestId,
          );
        } catch (error) {
          logger.error('Error setting feature cookies', error, {
            request_id: res.locals.requestId,
          });
        }
      }
      ...
      postSignInController({ req, res, oauthTokens: tokenSet, returnUrl });
    } catch (error) {
