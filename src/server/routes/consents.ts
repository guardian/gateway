...
export const consentPagesAbTest: ConsentPage[] = [
	{
		page: 'our_content',
...
const getConsentPages: (mvt: number) => ConsentPage[] = (mvt) => {
	const ab = abTestApiForMvtId(mvt);

	const isInABTestVariant = ab.isUserInVariant(
		abSimplifyRegistrationFlowTest.id,
		abSimplifyRegistrationFlowTest.variants[0].id,
	);
	if (isInABTestVariant) return consentPagesAbTest;
	else return consentPages;
};
router.get(
	'/consents',
...
router.get(
	...
	handleAsyncErrors(async (req: Request, res: ResponseWithRequestState) => {
		...
		const consentPages = getConsentPages(1);
