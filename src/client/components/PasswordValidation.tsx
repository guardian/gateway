...
import {passwordValidatorsCss, validationInfoCss, ValidationStyling, validationSymbol,} from "@/client/styles/PasswordValidationStyles";
import {isBrowser, isNode} from "@/shared/lib/isNode";

const sixCharactersRegex: RegExp = /^.{6,}$/;
...
const multipleErrorsText = "This password isn't valid. Please make sure it matches the required criteria";
...
const matchPassword = (password: string, passwordRepeated: string): PasswordMatches => {
  return {
    sixOrMore: {
...
export const passwordValidation = (password: string, passwordRepeated: string): ValidationResult => {
  let errorCount = 0;
...
export const PasswordValidationComponent = (props: PasswordValidationProps) => {
  ...
  let failureStyling: ValidationStyling = props.hasFailedToSubmit && isBrowser ? 'error' : 'failure';
  ...
  return (
    <div css={passwordValidatorsCss}>
      <div>
        {!isNode ? <div css={validationSymbol(lengthValidationStyling)}/> : null}
        <div css={validationInfoCss(lengthValidationStyling)}>{lengthValidationText}</div>
      </div>
      <div>
        <div css={validationSymbol(upperAndLowercaseValidationStyling)}/>
        <div css={validationInfoCss(upperAndLowercaseValidationStyling)}>{mixtureOfCaseText}</div>
      </div>
      <div>
        <div css={validationSymbol(symbolValidationStyling)}/>
        <div css={validationInfoCss(symbolValidationStyling)}>{symbolOrNumberText}</div>
      </div>
      <div>
