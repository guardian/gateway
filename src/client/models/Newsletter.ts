import { Newsletters, NewslettersWithImages } from '@/shared/model/Newsletter';
import {
	AFTERNOON_UPDATE_AU_SQUARE_IMAGE,
	BREAKING_NEWS_AU_SQUARE_IMAGE,
	CLEAR_AIR_AU_SQUARE_IMAGE,
	FEAST_SQUARE_IMAGE,
	FIVE_GREAT_READS_AU_SQUARE_IMAGE,
	HEADLINES_EU_SQUARE_IMAGE,
	HEADLINES_UK_SQUARE_IMAGE,
	HEADLINES_US_SQUARE_IMAGE,
	MORNING_MAIL_AU_SQUARE_IMAGE,
	SAVED_FOR_LATER_AU_SQUARE_IMAGE,
	TECH_SCAPE_SQUARE_IMAGE,
	THE_CRUNCH_AU_SQUARE_IMAGE,
	THE_GUIDE_IMAGE,
	THE_LONG_WAVE_SQUARE_IMAGE,
	THE_STAKES_US_SQUARE_IMAGE,
	THIS_IS_EUROPE_SQUARE_IMAGE,
	WELL_ACTUALLY_SQUARE_IMAGE,
} from '@/client/assets/newsletters';

export const NEWSLETTER_IMAGES: Record<NewslettersWithImages, string> = {
	[Newsletters.MORNING_MAIL_AU]: MORNING_MAIL_AU_SQUARE_IMAGE,
	[Newsletters.BREAKING_NEWS_AU]: BREAKING_NEWS_AU_SQUARE_IMAGE,
	[Newsletters.FIVE_GREAT_READS_AU]: FIVE_GREAT_READS_AU_SQUARE_IMAGE,
	[Newsletters.CLEAR_AIR_AU]: CLEAR_AIR_AU_SQUARE_IMAGE,
	[Newsletters.AFTERNOON_UPDATE_AU]: AFTERNOON_UPDATE_AU_SQUARE_IMAGE,
	[Newsletters.SAVED_FOR_LATER_AU]: SAVED_FOR_LATER_AU_SQUARE_IMAGE,
	[Newsletters.THE_CRUNCH_AU]: THE_CRUNCH_AU_SQUARE_IMAGE,
	[Newsletters.HEADLINES_US]: HEADLINES_US_SQUARE_IMAGE,
	[Newsletters.THE_STAKES_US]: THE_STAKES_US_SQUARE_IMAGE,
	[Newsletters.WELL_ACTUALLY]: WELL_ACTUALLY_SQUARE_IMAGE,
	[Newsletters.TECH_SCAPE]: TECH_SCAPE_SQUARE_IMAGE,

	[Newsletters.FIRST_EDITION]: TECH_SCAPE_SQUARE_IMAGE,
	[Newsletters.HEADLINES_UK]: HEADLINES_UK_SQUARE_IMAGE,
	[Newsletters.FEAST]: FEAST_SQUARE_IMAGE,
	[Newsletters.THE_GUIDE]: THE_GUIDE_IMAGE,
	[Newsletters.THE_LONG_WAVE]: THE_LONG_WAVE_SQUARE_IMAGE,
	// [Newsletters.THIS_WEEK_IN_TRUMPLAND]: HEADLINES_US_SQUARE_IMAGE,
	[Newsletters.THIS_IS_EUROPE]: THIS_IS_EUROPE_SQUARE_IMAGE,
	[Newsletters.HEADLINES_EUROPE]: HEADLINES_EU_SQUARE_IMAGE,
};
